...
export function Chat({
  ...
  const onEvent = useCallback(
    (event: unknown) => {
      ...
      if (e.eventType === "MESSAGE_CREATED") load();
    },
    [load],
  );
  useSubscription(`/topic/enquiries/${enquiryId}`, onEvent);
  async function send() {
    if (!text.trim()) return;
    try {
      const message = await enquiriesApi.send(enquiryId, {
        senderType,
        senderId,
        content: text.trim(),
      });
      setItems((old) =>
        old.some((x) => x.messageId === message.messageId)
          ? old
          : [...old, message],
      );
      setText("");
  ...
  return (
    <section className="chat">
      ...
      <div className="messages">
        {items.map((m) => (
          <div
            className={`message ${m.senderType === senderType ? "mine" : ""}`}
            key={m.messageId}
          >
            <b>{m.senderType === "CUSTOMER" ? "Customer" : "Agent"}</b>
            <span>{m.content}</span>
          </div>
        ))}
      </div>
